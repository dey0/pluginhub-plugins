...
public class CoxThievingDatabox extends OverlayPanel {
  ...
  public Dimension render(Graphics2D graphics) {
    ...
    if (plugin.gc_local == null || plugin.gc_local.num_opened == 0) {
      ...
      myindex = Arrays.binarySearch(plugin.gc_others, 0, plugin.gc_others_count, plugin.gc_local,
          plugin.comparator);
      if (myindex < 0) myindex = -myindex - 1;
    }
    ...
    elems.add(TitleComponent.builder().color(Color.WHITE)
        .text((sum_grubs == plugin.num_grubs ? "Grub count: " : "Est. grub count: ") + sum_grubs)
        .build());
    for (int i = 0; i < plugin.gc_others_count; i++) {
      if (i == myindex) add_gc_line(elems, plugin.gc_local);
      add_gc_line(elems, plugin.gc_others[i]);
    }
    if (myindex == plugin.gc_others_count) add_gc_line(elems, plugin.gc_local);
    return super.render(graphics);
  }

  private void add_gc_line(List<LayoutableRenderableEntity> elems, GrubCollection gc) {
